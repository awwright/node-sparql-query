...
Result.prototype.equals = function equals(results){
	...
	return self.results.every(function(resa, i){
		var resb = recordset[i];
		var k0 = Object.keys(resa);
		var k1 = Object.keys(resb);
		if(k0.length!==k1.length) return false;
		return k0.every(function(kn){
			if(resa[kn]===resb[kn]) return true;
			if(resa[kn].termType==='BlankNode') return bnodeMap.equals(resa[kn], resb[kn]);
			return resa[kn].equals && resa[kn].equals(resb[kn]);
		});
	});
}
